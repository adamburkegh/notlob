...
### PYZ archive (zero dependencies)

Notlob is released as a `.pyz` file, runnable with
...
Or create a Python venv, clone and build using pip in the usual way.



## Developing Notlob Projects

### Running

Two basic commands, each with subcommands and help. `notlob` is for build time. `lob` is for runtime and is an alias for `notlob run`.
...
Notlob was written at arm's length using Claude Code. This README is completely hand authored. Every other artifact, including [LANGUAGE.md](notlob/docs/LANGUAGE.md) and [DESIGN.md](notlob/docs/DESIGN.md), has been emitted or altered by the language extrusion machine as the result of dialogue.
